...
def company_info(request):
    ...
    if request.method == "POST":
        ...
        if not (
            ...
            messages.error(
                request,
                "No estás autorizado para modificar la información de contacto.",
            )
        ...
        if form.is_valid():
            ...
            messages.success(
                request, "Información de contacto actualizada correctamente."
            )
            return redirect("company_info")
        else:
        ...
        form = (
            ContactInfoForm(instance=contact)
            if (
                request.user.is_authenticated
                and (
