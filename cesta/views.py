...
def añadir_a_cesta(request, producto_id, cantidad=None):
    ...
    try:
        ...
        messages.success(
            request, f"Has añadido {cantidad} artículo(s) a la cesta correctamente."
        )
...
def checkout_view(request):
    ...
    try:
        pedido_fallido = (
            Pedido.objects.filter(cesta_asociada=cesta, estado=EstadoPedido.PENDIENTE)
            ...
            with transaction.atomic():
                ...
                if datos["metodo_pago"] == MetodoPago.CONTRAREEMBOLSO:
                    ...
                    try:
                        ...
                        print(
                            f"Email de contrareembolso enviado para el pedido {pedido.id}"
                        )
                    except Exception as e:
                    ...
                    return redirect(
                        "cesta:pedido_confirmacion", public_id=pedido.public_id
                    )
    ...
    try:
        ...
        session = stripe.checkout.Session.create(
            ...
            success_url=request.build_absolute_uri(
                reverse_lazy("cesta:pago_exito", kwargs={"public_id": pedido.public_id})
            ),
            cancel_url=request.build_absolute_uri(
                reverse_lazy("cesta:pago_fallo", kwargs={"public_id": pedido.public_id})
            ),
...
def es_admin(user):
    return user.is_authenticated and (
        user.is_superuser or getattr(user, "rol", "") == "ADMIN"
    )
...
def pedido_admin_detalle(request, public_id):
    ...
    if request.method == "POST":
        ...
        return redirect(
            reverse(
                "cesta:pedido_admin_detalle", kwargs={"public_id": pedido.public_id}
            )
        )

    return render(
        request,
        "cesta/pedido_admin_detalle.html",
        {
            "pedido": pedido,
